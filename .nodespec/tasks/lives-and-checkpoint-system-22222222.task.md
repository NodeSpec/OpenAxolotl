...
- [ ] **T5 — Expose the interface OpenAxolotl Game Client consumes, per Contract "Checkpoint and Life Interface" (dependency).** <!-- t:eafae80f -->
  ...
  Dependency contract — capture the reference/identifier wiring in this node's config artifacts; no payload schema expected.
  ↳ serves (unverified match): REQ-003 "Lives decrement only from designated catastrophic sources: pit volumes, crush hazards, boss finishers, and Dredger area-wipe attacks" — requirement not mapped to that node; verify or reassign before relying on it
  ↳ serves (unverified match): REQ-003 "Ordinary enemy and hazard contact never decrements lives" — requirement not mapped to that node; verify or reassign before relying on it
  ↳ serves (unverified match): REQ-003 "Reaching zero lives respawns the player at the last activated checkpoint with the life count replenished" — requirement not mapped to that node; verify or reassign before relying on it
  ↳ serves (unverified match): REQ-003 "Reaching zero lives never restarts the world from the beginning and never modifies or penalizes the save file" — requirement not mapped to that node; verify or reassign before relying on it
  ↳ serves (unverified match): REQ-003 "Activating a checkpoint records respawn position and restores both life count and capability state" — requirement not mapped to that node; verify or reassign before relying on it
  ↳ serves (unverified match): REQ-003 "A world can declare a lives-per-attempt override through the Level Contract, and the global default applies when it does not" — requirement not mapped to that node; verify or reassign before relying on it
  ↳ serves (unverified match): REQ-003 "Checkpoint spacing in every official world keeps the replay time from any checkpoint to the next at or below tuning key progression.max_retry_seconds" — requirement not mapped to that node; verify or reassign before relying on it
